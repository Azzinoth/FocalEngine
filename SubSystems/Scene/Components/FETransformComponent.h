...
namespace FocalEngine
{
	...
	public:
		...
		glm::vec3 GetPosition(FE_COORDINATE_SPACE_TYPE SpaceType = FE_LOCAL_SPACE) const;
		glm::vec3 GetRotation(FE_COORDINATE_SPACE_TYPE SpaceType = FE_LOCAL_SPACE) const;
		glm::quat GetQuaternion(FE_COORDINATE_SPACE_TYPE SpaceType = FE_LOCAL_SPACE) const;
		glm::vec3 GetScale(FE_COORDINATE_SPACE_TYPE SpaceType = FE_LOCAL_SPACE) const;

		void SetPosition(glm::vec3 NewPosition, FE_COORDINATE_SPACE_TYPE SpaceType = FE_LOCAL_SPACE);
		void SetRotation(glm::vec3 NewRotation, FE_COORDINATE_SPACE_TYPE SpaceType = FE_LOCAL_SPACE);
		void RotateAroundAxis(const glm::vec3& Axis, const float& RotationAmount, FE_COORDINATE_SPACE_TYPE SpaceType = FE_LOCAL_SPACE);
		void SetQuaternion(glm::quat Quaternion, FE_COORDINATE_SPACE_TYPE SpaceType = FE_LOCAL_SPACE);
		void RotateByQuaternion(glm::quat Quaternion);
		void SetScale(glm::vec3 NewScale, FE_COORDINATE_SPACE_TYPE SpaceType = FE_LOCAL_SPACE);
	...
	private:
		...
		void MoveAlongAxis(const glm::vec3& Axis, float MovementValue, FE_COORDINATE_SPACE_TYPE SpaceType = FE_LOCAL_SPACE);
