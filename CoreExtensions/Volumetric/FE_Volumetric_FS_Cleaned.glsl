...
//
// The look-up table contains 256 entries. Each entry is an sRGB triplet.
...
void main(void)
{
	...
	// stays in world-space units, so DistanceLimit can be compared against LinearDepth directly regardless of the volume scale.
	...
	float StepCount = 1000.0;
	float DistanceTravelled = 0;
	float StepSize = DistanceInsideBox / StepCount;
	float DistanceLimit = min(LinearDepth - DistanceToBox, DistanceInsideBox);

	// March through volume.
	float TotalDensity = 0;
	vec4 AccumulatedColor = vec4(0.0); // Accumulated color from the volume

	while (DistanceTravelled < DistanceLimit) {
		float SampleDistance = DistanceToBox + DistanceTravelled;
		vec3 SampleTextureCoordinates = LocalOrigin + LocalDirection * SampleDistance; // already in [0,1] inside the unit cube

		float VolumeValue = texture(volumeTexture, SampleTextureCoordinates).r;
		float VolumeValueMin = 0.0f;
		float VolumeValueMax = 0.1f;
		float NormalizedValue = (VolumeValue - VolumeValueMin) / (VolumeValueMax - VolumeValueMin);
		NormalizedValue = clamp(NormalizedValue, 0, 1);

		vec4 SampleColor = vec4(GetTurboColormapValue(NormalizedValue), 1.0f);

		float Density = NormalizedValue * 3.0f;

		// Skip empty space
		if (Density > 0.0) {
			// Calculate sample's contribution based on density
			float Absorption = Density * StepSize * 5.0;
			float SampleContribution = Absorption * exp(-TotalDensity);

			// Accumulate color and density
			AccumulatedColor += SampleContribution * SampleColor;
	...
	// Calculate final opacity based on accumulated density
	float Alpha = 1.0 - exp(-TotalDensity);

	// Blend the volume color with the scene color based on alpha
	out_Color = mix(out_Color, AccumulatedColor, Alpha);
}
