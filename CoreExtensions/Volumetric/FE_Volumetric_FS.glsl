...
void main(void)
{
	...
	//vec4 materialColors[4];
	//materialColors[0] = vec4(1.0, 0.0, 0.0, 1.0);
	//materialColors[1] = vec4(0.0, 1.0, 0.0, 1.0);
	//materialColors[2] = vec4(0.0, 0.0, 1.0, 1.0);
	//materialColors[3] = vec4(1.0, 1.0, 0.0, 1.0);
	//
	//float totalDensity = 0;
	...
	//
	//	float density = sampleData.r;
	//	float materialType = sampleData.g; // Assuming material type is stored in green channel
	//
	//
	//
	//
	...
	//		// This example assumes material type is stored as a value between 0-3 in the G channel
	//		int materialIndex = int(materialType * 4.0);
	//
	//
	//		// Fix that
	//		if (density > 0.15)
	...
	//
	//
	//		vec4 materialColor = materialColors[clamp(materialIndex, 0, 3)];
	//
	...
	//		volumeColor += sampleContribution * materialColor;
	...
	//out_Color = mix(out_Color, volumeColor, alpha);
	...
	while (dstTravelled < dstLimit) {
		...
		float VolumeValueMax = 35.3792000f;
		//float VolumeValueMin = 33.3936234f;
		//float VolumeValueMax = 35.1951752f;
		//float VolumeValueMin = 22.0f;
		//float VolumeValueMax = 55.0f;
		float NormalizedValue = (VolumeValue - VolumeValueMin) / (VolumeValueMax - VolumeValueMin);
		NormalizedValue = clamp(NormalizedValue, 0, 1);

		vec4 VolumeColor = vec4(getTurboColormapValue(NormalizedValue), 1.0f);


		//float Density = sampleData.a;
		float Density = NormalizedValue * 3.0f;

		//float ColorCombination = sampleData.r + sampleData.g + sampleData.b;
		...
		if (Density > 0.0) {
			...
			vec4 materialColor = VolumeColor;
			//vec4 materialColor = texture(volumeTexture, texCoords);
